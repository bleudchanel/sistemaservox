    ...
    public partial class FrmDocumento : PlantillaBase
    {
        ...
        private async void toolGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string metodoApi;
                _documento.FechaEmision = new DateTime(2018, 10, 10).ToString("dd/MM/yyyy");
        ...
        public async Task<string> GenerarXMLOut(DocumentoElectronico documentoElectronico)
        {
            var proxy = new HttpClient { BaseAddress = new Uri(ConfigurationManager.AppSettings["UrlOpenInvoicePeruApi"]) };

            string metodoApi;
            //_documento.FechaEmision = new DateTime(2018, 10, 10).ToString("dd/MM/yyyy");
            switch (documentoElectronico.TipoDocumento)
            {
                case "07":
                    metodoApi = "api/GenerarNotaCredito";
                    break;
            ...
            var response = await proxy.PostAsJsonAsync(metodoApi, documentoElectronico);
            var respuesta = await response.Content.ReadAsAsync<DocumentoResponse>();
            if (!respuesta.Exito)
                throw new ApplicationException(respuesta.MensajeError);

            RutaArchivo = Path.Combine("C:/DocumentosServox/",
                $"{documentoElectronico.IdDocumento}.xml");
